...
createApp({
    setup() {
        ...
        const storeCategory = async () => {
            ...
            } catch (err) { console.error('新增分類失敗', err); }
        };

        const storeCategoryAndClose = async () => {
            if (!newCategoryName.value.trim()) return;
            try {
                const res = await authFetch('/api/categories', {
                    method: 'POST',
        ...
        const storeAttraction = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    fetchAttractions(1);
                } else { const data = await res.json(); alert(data.message || '新增失敗'); }
            } catch (err) { console.error('新增景點錯誤', err); }
        };
        ...
        const updateAttraction = async () => {
            try {
                ...
                } else { const data = await res.json(); alert(data.message || '更新失敗'); }
            } catch (err) { console.error('更新景點錯誤', err); }
        };
        ...
        const deleteAttraction = async (id) => {
            ...
            } catch (err) { console.error('刪除景點失敗', err); }
        };
        ...
        const removeFavorite = async (item) => {
            try {
                const res = await authFetch(`/api/attractions/${item.id}/favorite`, { method: 'POST' });
                if (res.ok) favorites.value = favorites.value.filter(fav => fav.id !== item.id);
